...
/* ============================================================
   THE CANON QUEST, a gamified journey through the canon of
   ...
   real book spines that fill with color as you finish them.
   ============================================================ */
...
const PATHS = [
  ...
  {id:"society",name:"Understand Society",focus:"Politics · Economics · History · Power",
   ...
   books:[73,74,83,76,78,79,85,80,72]},
...
const LINKS = {
  ...
  43:[[38,"Hobbes justified absolute power to escape chaos; Locke took the same starting point and arrived at limited government instead."]],
  38:[[44,"Locke grounded rights in property and consent; Rousseau pushed the same social-contract logic toward full popular sovereignty."]],
...
const ACH_DEFS = [
  ...
  {id:"half",icon:"⚖️",name:"Halfway There",desc:`Finish half of the ${BOOKS.length}`,test:s=>s.done>=Math.ceil(BOOKS.length/2)},
  {id:"century",icon:"👑",name:"Canon Complete",desc:"Complete the entire canon",test:s=>s.done>=BOOKS.length}
];
...
/* Cover palette is now derived from the book's genre color (GENRE_COLORS),
   the same signal used for shelf spines and timeline chips, so color means
   one consistent thing everywhere: subject, not era. */
...
function mix(hex,target,amt){ // amt 0..1, moves hex toward target (also hex)
  const a=hexToRgb(hex), t=hexToRgb(target);
  return rgbToHex(a.r+(t.r-a.r)*amt, a.g+(t.g-a.g)*amt, a.b+(t.b-a.b)*amt);
}
const paletteCache={};
function paletteFor(genre){
  if(paletteCache[genre]) return paletteCache[genre];
  const base=GENRE_COLORS[genre]||"#8F7638";
  const p={
    bg: mix(base,"#0A0F0C",0.82),   // deep, dark tint of the genre color for the cover background
    accent: base,                    // the genre color itself, true
    hi: mix(base,"#FFFFFF",0.55),   // lightened genre color for the highlight motif
  };
  paletteCache[genre]=p;
  return p;
}
...
function Cover({b,big}){
  ...
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{borderRadius:"2px 5px 5px 2px",boxShadow:"3px 4px 10px rgba(0,0,0,.5)",flexShrink:0}}>
      ...
      <rect x="1" y="0" width="1" height={h} fill={pal.accent} opacity=".4"/>
      <g opacity=".18"><Motif k={motif} w={w} h={h} c={pal.hi}/></g>
      <Motif k={motif} w={w} h={h-h*.3} c={pal.accent}/>
      <rect x="0" y={h-20} width={w} height={20} fill="rgba(0,0,0,.45)"/>
      <text x={w/2} y={h-6} textAnchor="middle" fontFamily="'IBM Plex Mono',monospace" fontSize={big?6.5:5.5} fill={pal.hi} opacity=".85" letterSpacing="0.5">
...
export default function CanonQuest(){
  ...
  const nextRec=useMemo(()=>{
    ...
    BOOKS.filter(unread).forEach(b=>{
      ...
      let score=shared.length*2 + (genresDone.has(b.g)?1:1.6) - Math.abs(b.d-(avgD+0.4));
  ...
  const quickPick=mode=>{
    ...
    if(mode==="think"){ const ideaGenres=new Set(["Philosophy","Psychology","Politics"]); pick=[...unread].filter(b=>ideaGenres.has(b.g)).sort((a,b)=>a.d-b.d)[0]; why="A book built to rearrange how you see the world, start here."; }
    if(pick){ setOpenId(pick.id); toast(`📚 Suggested: “${pick.t}”, ${why}`); }
  };

  /* ---------- VIEWS ---------- */
  const pct=Math.round(100*S.done/BOOKS.length); // real percentage, not raw count
  const goalCard=()=>{
    const g=st.goal;
        ...
        {!g? (
          <div>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              ...
              <button className="cq-btn sm" onClick={()=>setGoal({type:"fiveyr",label:`All ${BOOKS.length} in 5 years`})}>{BOOKS.length} in 5 years</button>
        ...
        <div style={{display:"flex",gap:22,alignItems:"center",flexWrap:"wrap",padding:"30px 4px 6px"}}>
          ...
          <div style={{flex:1,minWidth:230}}>
            ...
            <div style={{marginTop:10,maxWidth:420}}>
              ...
              <div className="cq-mono" style={{fontSize:11,color:"var(--gilt)",marginTop:6}}>{S.done} / {BOOKS.length} books completed</div>
            </div>
          </div>
        </div>
  ...
  const Library=()=>{
    ...
    return (
      <div>
        <div className="cq-rule" style={{marginTop:26}}><h2>The Library · all {BOOKS.length} volumes</h2></div>
        ...
        <div className="cq-mono" style={{fontSize:11,color:"var(--muted-text)",marginBottom:12}}>{list.length} of {BOOKS.length} shown</div>
    ...
    <div className="cq-root">
      ...
      <div className="cq-wrap">
        <nav className="cq-nav">
          <div className="cq-brand">The Canon Quest <small>· {BOOKS.length} books that shaped the world</small></div>
          <div style={{flex:1}}/>
          {TABS.map(([id,label])=>
